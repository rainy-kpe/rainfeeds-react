...
const initialState: ICardState = {
  cards: [
    { type: "rss", order: 1, title: "Reddit", updateRate: 15, urls: ["http://www.reddit.com/.rss"] },
...
export const cardReducer = (state: ICardState = initialState, action: ICardAction | IAddCardAction) => {
  switch (action.type) {
    ...
    case "UPDATE_CARD":
      return { ...state, cards: state.cards.map(card => (card.title === action.card.title ? action.card : card)) }
    case "CLEAR_CARDS":
      return { ...state, cards: [] }
...
export const getCardsFromDatabase = () => {
  return async (dispatch: Dispatch<ICardAction | IAddCardAction>) => {
    dispatch(clearCards())
